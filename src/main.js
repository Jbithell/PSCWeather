...
function cacheSerial(message) {
  //Build up a bit of a cache of serial messages because they seem to come in chunks
  if (serialCache["updated"] < Date.now() - 65000) {
    //If the cache is older than 65 seconds
  ...
  if (offset !== false) {
    // If we have a full message then parse it, otherwise wait and let the cache build up a bit
    const response = responseParser(serialCache["cache"], offset); //Parse the message
    serialCache = {
      //Reset the serialCache
      cache: Buffer.from([]),
      updated: Date.now(),
    };
    if (response) {
